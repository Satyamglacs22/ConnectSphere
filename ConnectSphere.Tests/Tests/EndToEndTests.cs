...
namespace ConnectSphere.Tests.Tests
{
    ...
    public class EndToEndTests
    {
        ...
        public async Task FullSocialFlow_RegisterFollowPostLikeComment()
        {
            ...
            var reg1 = await _client.PostAsync("/api/users/register", user1);
            reg1.StatusCode.Should().Be(System.Net.HttpStatusCode.OK);

            var reg1Json = JObject.Parse(await reg1.Content.ReadAsStringAsync());
            ...
            var reg2 = await _client.PostAsync("/api/users/register", user2);
            var reg2Json = JObject.Parse(await reg2.Content.ReadAsStringAsync());
            ...
            var login1Json = JObject.Parse(await login1.Content.ReadAsStringAsync());
            ...
            // ── Step 7: Switch back to User 1 ─────────────────────────────
            _client.SetToken(token1);

            // ── Step 8: User 1 likes User 2's post ────────────────────────
            var likeResponse = await _client.PostAsync(
                "/api/likes/toggle",
                new { UserId = user1Id, TargetId = postId, TargetType = "POST" });
            likeResponse.StatusCode.Should().Be(System.Net.HttpStatusCode.OK);

            var likeJson = JObject.Parse(await likeResponse.Content.ReadAsStringAsync());
            ...
            var commentResponse = await _client.PostAsync(
                "/api/comments",
                new { PostId = postId, UserId = user1Id, Content = "Great post!", ParentCommentId = (int?)null });
            commentResponse.StatusCode.Should().Be(System.Net.HttpStatusCode.OK);

            // ── Step 10: Verify post has likes and comments ────────────────
            var getPostResponse = await _client.GetAsync($"/api/posts/{postId}");
            var getPostJson = JObject.Parse(await getPostResponse.Content.ReadAsStringAsync());

            getPostJson["likeCount"]!.Value<int>().Should().BeGreaterThan(0);
            getPostJson["commentCount"]!.Value<int>().Should().BeGreaterThan(0);
        }
    }
}
