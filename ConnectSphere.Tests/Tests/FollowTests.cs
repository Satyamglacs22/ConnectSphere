...
namespace ConnectSphere.Tests.Tests
{
    ...
    public class FollowTests
    {
        ...
        {
            var response = await _client.PostAsync("/api/follows", TestData.FollowRequest(_user1Id, _user2Id));
        ...
        {
            await _client.PostAsync("/api/follows", TestData.FollowRequest(_user1Id, _user2Id));

            var response = await _client.PostAsync("/api/follows", TestData.FollowRequest(_user1Id, _user2Id));

            var json = JObject.Parse(await response.Content.ReadAsStringAsync());
            json["message"]!.ToString().Should().Contain("Already following");
        }
        ...
        public async Task GetFollowers_ReturnsFollowersList()
        {
            ...
            var response = await _client.GetAsync($"/api/follows/{_user2Id}/followers");
        ...
        public async Task IsFollowing_AfterFollow_ReturnsTrue()
        {
            ...
            var response = await _client.GetAsync($"/api/follows/isFollowing/{_user1Id}/{_user2Id}");
        ...
        public async Task Unfollow_AfterFollow_ReturnsSuccess()
        {
            ...
            var response = await _client.DeleteAsync($"/api/follows/{_user2Id}");
